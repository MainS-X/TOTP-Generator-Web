...
验证码在浏览器内计算。通过 HTTP/HTTPS 访问时，页面仅在可见、设备在线且输入了格式有效的密钥时向本站发送 `HEAD` 请求，读取 `Date` 响应头校准时间；请求不包含输入的密钥或验证码。

- 首次使用时取 3 个时间样本，此后每 30 分钟重新校准一次。校准有效期间，输入和复制会复用已有时间基准。
- 页面隐藏、离线或密钥被清空/格式无效时暂停校时，并取消进行中的请求；恢复使用时按需继续。
- 校时失败后按 1、2、4、8、16、30 分钟的间隔重试，之后保持 30 分钟；输入、焦点和联网事件不会跳过失败等待时间。单个请求 5 秒超时。
- 首次校时失败或直接打开本地文件时使用设备时间；已有校准记录时继续使用此前的时间基准。请保持系统时间准确。

## 本地检查

安装 Node.js 后运行 `node --test tests/time-sync.test.cjs`，验证校时请求量、后台暂停、失败重试及验证码复制。测试使用模拟时钟和网络，不会访问线上服务。
